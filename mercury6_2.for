...
c
c      write (23,231) mem(58)(1:lmem(58)), 
c     %  abs((en(2) + en(3) - en(1)) / en(1))
c      write (23,232) mem(59)(1:lmem(59)), 
c     %  abs((am(2) + am(3) - am(1)) / am(1))
c      write (23,231) mem(60)(1:lmem(60)), abs(en(3) / en(1))
c      write (23,232) mem(61)(1:lmem(61)), abs(am(3) / am(1))
c      close (23)
...
C MCL shared with C++
      integer integrating
      real*4 gtime
      real*4 x0(NMAX), y0(NMAX), z0(NMAX), u0(NMAX), v0(NMAX), w0(NMAX)
      real*4 q0(NMAX), e0(NMAX), i0(NMAX), p0(NMAX), n0(NMAX), l0(NMAX)
      common /shared/ integrating, gtime, x0, y0, z0, u0, v0, w0,
     % q0, e0, i0, p0, n0, l0
...
c Save the current heliocentric coordinates and velocities
     ...
     %    opt)
      ...
      call mco_iden0(time,jcen,nbod,nbig,h0,m,xh0,vh0,ngf,ngflag,opt)
...
C MCL
      ...
      do j=2,nbod
         call mco_x2el0 (temp,x0(j),y0(j),z0(j),
     %   u0(j),v0(j),w0(j),
     %   q0(j), e0(j), i0(j), p0(j), n0(j), l0(j))
      end do
...
c Write message to info file (I=0 implies collision with the central body)
  10  open (23, file=outfile, status='old')
c
      if (opt(3).eq.1) then
...
c  50  open (22, file=outfile, status='old', access='append')
...
c is in progress, store details
 ...
 230            open (23,file=outfile,status='old')
...
c
      ...
      subroutine mco_iden0 (time,jcen,nbod,nbig,h,m,xh,vh,ngf,ngflag,
     %  opt)
c
      implicit none
      include 'mercury.inc'
...
c Input/Output
      integer nbod,nbig,ngflag,opt(8)
      real*8 time,jcen(3),h,m(nbod),xh(3,nbod), vh(3,nbod),ngf(4,nbod)
c
c Local
      integer j
...
C MCL
c
      subroutine mco_x2el0 (gm,x,y,z,u,v,w,q,e,i,p,n,l)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      real*4 gm
      real*4 q,e,i,p,n,l,x,y,z,u,v,w
c
c Local
      real*4 hx,hy,hz,h2,h,v2,r,rv,s,true
      real*4 ci,to,temp,tmp2,bige,f,cf,ce
      real*4 TWOPIsingle
...
c True longitude
      if (hy.ne.0) then
        ...
        temp = (1.d0 - ci) * to
        tmp2 = to * to
        true = atan2((y*(1.e0+tmp2*ci)-x*temp),(x*(tmp2+ci)-y*temp))
      else
        true = atan2(y * ci, x)
      end if
      if (ci.lt.0) true = true + PI
...
c Mean anomaly for ellipse
        if (e.lt.1) then
          if (abs(ce).gt.1) ce = sign(1.e0,ce)
          bige = acos(ce)
          if (rv.lt.0) bige = TWOPIsingle - bige
          l = bige - e*sin(bige)
        else
c
c Mean anomaly for hyperbola
...
c Longitude of perihelion
        cf = (s - r) / (e*r)
        if (abs(cf).gt.1) cf = sign(1.e0,cf)
        f = acos(cf)
        if (rv.lt.0) f = TWOPIsingle - f
        p = true - f
        p = mod (p + TWOPIsingle + TWOPIsingle, TWOPIsingle)
      end if
c
      if (l.lt.0) l = l + TWOPIsingle
      if (l.gt.TWOPIsingle) l = mod (l, TWOPIsingle)
c
c------------------------------------------------------------------------------
c
      return
...
c If required, output the stored close encounter details
  ...
  10    open (22, file=outfile(2), status='old')
        do k = 1, nstored
...
c If new encounter minima have occurred, decide whether to stop integration
  ...
  20    open (23, file=outfile(3), status='old')
...
c Dump data for the Big (i=1) and Small (i=2) bodies
  ...
  20        open (31, file=c(1:12), status='unknown')
          else
  25        open (31, file=dumpfile(i), status='old')
          end if
c
...
c Dump the integration parameters
  40    if (idp.eq.1) open (33,file='param.tmp',status='unknown')
  45    if (idp.eq.2) open (33, file=dumpfile(3), status='old')
c
...
c Create new version of the restart file
  60    if (idp.eq.1) open (35, file='restart.tmp', status='unknown',
     %    err=60)
  65    if (idp.eq.2) open (35, file=dumpfile(4), status='old')
...
c Check if information file exists, and append a continuation message
 ...
 320    open(23,file=outfile(3),status='old')
      else
c
c If new integration, check information file doesn't exist, and then create it
     ...
     %    ' ',1,outfile(3),80)
c 410    open(23, file = outfile(3), status = 'new')
 410    open(23, file = outfile(3), status = 'new')
      end if
c
c------------------------------------------------------------------------------
c
...
c Check if the file containing integration parameters exists, and open it
     ...
     %  ' ',1,filename,80)
  30  open  (13, file=filename, status='old')
c
...
c Check if the file containing data for Big bodies exists, and open it
 ...
 110    open (11, file=filename, status='old')
c
c Read data style
...
C MCLc Check if another object has the same name
C MCL         do k = 1, nbod - 1
C MCL           if (id(k).eq.id(nbod)) call mio_err (23,mem(81),lmem(81),
C MCL      %      mem(103),lmem(103),id(nbod),8,' ',1)
C MCL         end do
C MCL 
...
c Read in energy and angular momentum variables, and convert to internal units
 330    open (35, file=dumpfile(4), status='old')
          read (35,*) opflag
...
c Check that element and close-encounter files don't exist, and create them
     ...
     %      ' ',1,outfile(j),80)
 430      open  (20+j, file=outfile(j), status='new')
          close (20+j)
        end do
c
c Check that dump files don't exist, and then create them
        do j = 1, 4
          ...
          if (test) call mio_err (23,mem(81),lmem(81),mem(87),lmem(87),
     %      ' ',1,dumpfile(j),80)
 450      open  (30+j, file=dumpfile(j), status='new')
          close (30+j)
        end do
c
...
c Open the orbital-elements output file
  10  open (21, file=outfile, status='old')
c
c------------------------------------------------------------------------------
c
...
c Write message to information file
  20      open  (23,file=outfile,status='old')
          if (opt(3).eq.1) then
            call mio_jd2y (time,year,month,t1)
            flost = '(1x,a8,a,i10,1x,i2,1x,f8.5)'
