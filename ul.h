...
#define ulIsBigEndian    (*((char *) &_ulEndianTest ) == 0)
 ...
 void ulEndianSwap(unsigned int *x)
{
  *x = (( *x >> 24 ) & 0x000000FF ) | 
       (( *x >>  8 ) & 0x0000FF00 ) | 
       (( *x <<  8 ) & 0x00FF0000 ) | 
 ...
 void ulEndianSwap(unsigned short *x)
{
  *x = (( *x >>  8 ) & 0x00FF ) | 
       (( *x <<  8 ) & 0xFF00 ) ;
}
 ...
 void ulEndianSwap(float *x) { ulEndianSwap((unsigned int   *)x); }
 void ulEndianSwap(int   *x) { ulEndianSwap((unsigned int   *)x); }
 void ulEndianSwap(short *x) { ulEndianSwap((unsigned short *)x); }
