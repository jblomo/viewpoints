...
void make_main_menu_bar()
{
  ...
  // KLUDGE ALERT!  Add an offset to the integer referenced by the user_data
  // pointer to indicate a different write mode
  main_menu_bar->add( 
...
{
  // Evaluate user_data to get file format
  if( (int) user_data == BINARY) dfm.ascii_input( 0);
  else dfm.ascii_input( 1);

  // Query user to get and open input file
  int iQueryStatus = 0;
